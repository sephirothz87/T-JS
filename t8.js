...
(function(window, document) {
	...
	KTPlayer.prototype = {
		...
		bind: function() {
			...
			video.addEventListener('timeupdate', this.updateTime);  //当目前的播放位置已更改时
			this.target.querySelector('.ktPlayer-icon-volume').addEventListener('click', this.mute);//是否静音切换
			video.addEventListener('ended', this.end);   //当目前的播放列表已结束时
			this.bindSpeedRate();//设置或返回音频/视频播放的速度
			this.target.querySelector('.ktPlayer-icon-fullscreen').addEventListener('click', this.fullScreen);//全屏切换
			
			video.addEventListener('playing', this.loading);   //当音频/视频在已因缓冲而暂停或停止后已就绪时
			this.target.querySelector('.ktPlayer-bar-wrap').addEventListener('click', this.clickProgressBar);  //设置从当前位置播放
			this.target.querySelector('.ktPlayer-thumb').addEventListener('mousedown', this.dragProgressThumb);  //拖动进度条
			this.target.querySelector('.ktPlayer-volume-bar').addEventListener('click', this.clickVolumeBar);  //设置音量
			this.target.querySelector('.ktPlayer-setting').addEventListener('mouseenter', this.SVGSettingMouseenter);   //设置按钮
			this.target.querySelector('.ktPlayer-setting').addEventListener('mouseleave', this.SVGSettingMouseleave);   //设置按钮
			this.target.querySelector('.ktPlayer-icon-fullscreen').addEventListener('mouseenter', this.SVGFullscreenMouseenter);  //全屏按钮
			this.target.querySelector('.ktPlayer-icon-fullscreen').addEventListener('mouseleave', this.SVGFullscreenMouseleave);  //全屏按钮
			this.target.addEventListener('mousemove', this.hideController);  //控制按钮列表显示切换
			this.target.addEventListener('mouseleave', this.hideController);  //控制按钮列表显示切换
			this.target.querySelector('.ktPlayer-controller').addEventListener('mouseenter', this.controllerMouseenter);   //控制按钮列表
			this.target.querySelector('.ktPlayer-controller').addEventListener('mouseleave', this.controllerMouseleave);   //控制按钮列表
			
			document.addEventListener('keydown', this.keyBoard);
			document.addEventListener("fullscreenchange", this.fullscreenchange);   //全屏切换监听
		...
		load: function() {   // video.buffered返回表示音频/视频已缓冲部分的 TimeRanges 对象
		...
		fullScreen: function() {
			...
			if(!fullscreenElement) {  //如果不是全屏
				if(_this.target.requestFullscreen) {
					_this.target.requestFullscreen();    //requestFullscreen() 请求进入全屏模式。
				...
				if(document.exitFullscreen) {
					document.exitFullscreen();    //exitFullscreen() 退出全屏模式。
		...
		changeVolumeBar: function(value) {   //音量控制
		...
		clickProgressBar: function(e) {
			...
			video.currentTime = parseFloat(clickPercent * video.duration); //设置或返回音频/视频中的当前播放位置（以秒计）
		},
		...
		clickVolumeBar: function(e) {
			...
			video.volume = clickPercent;   //设置或返回音频/视频的音量
